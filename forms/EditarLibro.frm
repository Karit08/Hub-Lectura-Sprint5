...
Private Sub editar_Click(Index As Integer)
    ...
    If cantidadTitulo = 0 Then
        ...
        If Dir(imgPath) <> "" Then
            ...
            Set Rec = New ADODB.Recordset
            Dim updateLibro As String
            updateLibro = "UPDATE catalogo SET " & _
              "id_genero = '" & identificacion_genero & "', " & _
              "titulo = '" & Titulo & "', " & _
              "imagen = '" & imagen & "', " & _
              "descripcion = '" & descripcion & "', " & _
              "autor = '" & autor & "', " & _
              "editorial = '" & editorial & "', " & _
              "enlace = '" & enlace & "' " & _
              "WHERE id = " & idLibro
            Con.Execute updateLibro
            
            MsgBox "Libro editado exitosamente"
...
Private Sub Form_Load()
    ...
    ' Inicializamos variables, aquí puedes asignar valores según tus necesidades
    Dim Titulo As String
    Dim autor As String
    Dim Genero As String
    Dim editorial As String
    ...
    Set Rec = New ADODB.Recordset
    Dim sqlQuery As String
    Dim idTablaGenero As String
    sqlQuery = "SELECT * FROM catalogo WHERE titulo = '" & identificador & "';"
    Rec.Open sqlQuery, Con, adOpenStatic, adLockReadOnly
    
    idTablaGenero = Rec.Fields("id_genero").Value
    idLibro = Rec.Fields("id").Value
    
    'Set Rec2 = New ADODB.Recordset
    'Dim sqlGeneroQuery As String
    'Dim idGeneroRecuperado As String
    'sqlGeneroQuery = "SELECT id FROM genero WHERE nombre = '" & idTablaGenero & "';"
    'Rec2.Open sqlGeneroQuery, Con, adOpenStatic, adLockReadOnly
    
    'idGeneroRecuperado = Rec2.Fields("id").Value
    
    If Not Rec.EOF Then
        Text1.Text = Rec.Fields("titulo").Value
        Text2.Text = Rec.Fields("autor").Value
        Combo1.ListIndex = idTablaGenero - 1
        Text3.Text = Rec.Fields("editorial").Value
        Text4.Text = Rec.Fields("descripcion").Value
        Text5.Text = Rec.Fields("imagen").Value
        Text6.Text = Rec.Fields("enlace").Value
    Else
        MsgBox "No se encontró ningún libro con el título especificado.", vbExclamation
    End If
