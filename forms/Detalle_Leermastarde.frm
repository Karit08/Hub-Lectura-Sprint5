VERSION 5.00
Begin VB.Form DetalleLeerMasTarde 
   ...
   Begin VB.CommandButton luego 
      BackColor       =   &H00C0E0FF&
      Caption         =   "Eliminar de Leer más tarde"
      BeginProperty Font 
         Name            =   "Arial"
         Size            =   9.75
         Charset         =   0
   ...
   End
End
Attribute VB_Name = "DetalleLeerMasTarde"
...
Private Sub luego_Click()
    ...
    ' Consulta SQL para ver si no hay registros similares
    sqlQuery = "DELETE FROM contenido_leer_mas_tarde WHERE id_usuario = '" & idRecibido & "' AND id_libro = '" & id & "';"
    Rec.Open sqlQuery, Con, adOpenForwardOnly, adLockReadOnly

    MsgBox "Contenido eliminado de tu lista para ver mas tarde"
    Dim frm As New Leermastarde
    Unload Me
    frm.idRecibido = idRecibido
    frm.nombre = nombre
...
Private Sub eliminar_Click(Index As Integer)
    ...
    If contador = 0 Then
        ...
        Else
            MsgBox "Este contenido no puede ser agregado a tu lista de libros no deseados porque ya está en tu lista de libros favoritos"
        End If
    ...
    Else
        MsgBox "Este contenido ya está agregado a tu lista de libros no deseados"
    End If
